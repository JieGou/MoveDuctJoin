...
namespace MoveDuctJoin
{
    ...
    public class CmdDisconnect : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                Reference r = sel.PickObject(ObjectType.Element, new DuctSelectionFilter(), "Please pick a duct at the " + "connection to move.");
                duct = doc.GetElement(r.ElementId) as Duct;
                pFrom = r.GlobalPoint;
                r = sel.PickObject(ObjectType.Element, new DuctSelectionFilter(), "Please pick a target point on the " + "duct to move the connection to.");
                pTo = r.GlobalPoint;
            }
            ...
            Debug.Assert(con.Origin.IsAlmostEqualTo(cs.Origin), "expected same origin");
            Line line = Line.CreateUnbound(cs.Origin, cs.BasisZ);
            IntersectionResult ir = line.Project(pTo);
            pTo = ir.XYZPoint;
            Debug.Assert(line.Distance(pTo) < 1e-9, "expected projected point on line");
    ...
    public class CmdReconnect : IExternalCommand
    {
        ...
        /// </summary>
        private static Connector GetConnectedConnector(Connector con)
        {
            ...
            foreach (Connector c in refs)
            {
                ...
                if (c.ConnectorType == ConnectorType.End && !ownerId.Equals(c.Owner.Id.IntegerValue))
                {
                    neighbour = c;
                    break;
                }
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            Connector neighbour = GetConnectedConnector(con);
            // Modify document within a transaction
            using (Transaction tx = new Transaction(doc))
            {
                tx.Start("Move Fitting");
                ElementTransformUtils.MoveElement(doc, neighbour.Owner.Id, v);
                tx.Commit();
            }
            return Result.Succeeded;
        }
